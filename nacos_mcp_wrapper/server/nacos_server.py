		...
		try:
			...
			if server_detail_info is not None:
				...
				if self._nacos_settings.SERVICE_REGISTER and (
					...
					if self._nacos_settings.SERVICE_EPHEMERAL:
						await self._nacos_ai_service.register_mcp_server_endpoint(
								RegisterMcpServerEndpointParam(
										mcp_name=self.name,
										version=self.version,
										address=self._nacos_settings.SERVICE_IP,
			...
			_server = None
			try:
				_server = await self._nacos_ai_service.get_mcp_server(GetMcpServerParam(
						mcp_name=self.name,
						version=self.version
				))
